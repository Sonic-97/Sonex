...
import { LogOut, User, Navigation, Copy } from 'lucide-react';
...
export default function DriverSettingsPage() {
  ...
  useEffect(() => {
    setDriverId(sessionStorage.getItem('driver_id') || '');
    setDriverName(sessionStorage.getItem('driver_name') || '');
  }, []);
      ...
      <div className="bg-white rounded-2xl border border-[#E8E1D9] shadow-sm divide-y divide-[#E8E1D9]">
        <div className="p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="text-xs text-gray-400">معرف السائق</div>
            <div className="text-sm font-bold flex items-center gap-2">
              {driverId || '—'}
              {driverId && (
                <button onClick={() => copyToClipboard(driverId)} className="text-gray-300 hover:text-gray-500">
                  <Copy size={14} />
                </button>
              )}
            </div>
